...
public class Polynom implements Polynom_able {
	...
	public ArrayList<Point> findMinMax(double x0, double x1) {
		...
		if (!suspected.isEmpty()) {
			outPut = sortPoints(suspected);
		}
		findGlobalP(x0, x1, outPut);
		return outPut;
	}
	...
	// Helping method which is used to remove zero value Monoms from the list and
	// sum same power Monoms
	private void fixUp() {
		Iterator<Monom> it = this.iteretor();
	...
	private ArrayList<Point> sortPoints(ArrayList<Double> arr) {
		...
		while (input.hasNext()) {
			...
			if (der.f(p.getX() - eps) > 0 && der.f(p.getX() + eps) < 0)
				p.setMax(true); // sort if min or max
			else
				p.setMin(true);
	...
	private void findGlobalP(double x0, double x1, ArrayList<Point> points) {
		...
		if (hadLocalExm) { // only checks it if the arraylist had point
			while (it.hasNext()) {
				...
				if (currP.isGlobalMax() && newMax.getY() > currP.getY()) { // if the current point is global max checks
																			// if
	...
	private void findMax(ArrayList<Point> points) { // assumes has atleast one point
		...
		while (it.hasNext()) {
			...
			if (maxP.getY() < temp.getY()) {
				...
				Iterator<Point> it2 = points.iterator();
				while (it2.hasNext())
					it2.next().setGlobalMax(false); // resets all previous Global mins
				maxP.setGlobalMax(true);
			} else if (maxP.getY() == temp.getY())
				temp.setGlobalMax(true);
		}
	}
	...
	private ArrayList<Double> localExtremum(double x0, double x1) {
		...
		while (currentX <= x1) {// as long as we're in the given borders
			if (dev.f(currentX) * dev.f(currentX + eps) < 0) {// in case one point is negative while the other is
																// positive
				if (dev.derivative().f(currentX) != eps && dev.derivative().f(currentX) != -eps) {// checking whether
																									// this
																									// isn't a
																									// inflection
																									// point
